...
struct RoutesMapBackgroundView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, MKMapViewDelegate {
        ...
        var vehicleAnnotations: [String: MetroVehicle] = [:]
        ...
        func updateVehicleAnnotations(in mapView: MKMapView) {
            ...
            // Remover anotaciones obsoletas
            let currentVehicleAnnotations = mapView.annotations.compactMap { $0 as? MetroVehicle }
            let outdatedAnnotations = currentVehicleAnnotations.filter { metroViewModel.vehicles[$0.id] == nil }
            if !outdatedAnnotations.isEmpty {
                mapView.removeAnnotations(outdatedAnnotations)
            }
            
            // Actualizar o añadir nuevas anotaciones
            for (id, vehicle) in metroViewModel.vehicles {
                let existingAnnotations = currentVehicleAnnotations.filter { $0.id == id }
                
                if let existingAnnotation = existingAnnotations.first {
                    // Ya existe, actualizar su posición con animación
                       ...
                       existingAnnotation.coordinate.longitude != vehicle.longitude {
                        ...
                        if let annotationView = mapView.view(for: existingAnnotation) as? MetroAnnotationView,
                           let previousCoordinate = vehicle.previousCoordinate {
                            // Animar el cambio
                            animateVehicleMovement(annotation: existingAnnotation,
                    ...
                    // No existe, añadir nueva anotación
